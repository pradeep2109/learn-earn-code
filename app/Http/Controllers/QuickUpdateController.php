...
class QuickUpdateController extends Controller
{
   public function courseQuick(Request $request)
    ...
    public function userQuickSubscribe($id)
    {
        $users = User::findorfail($id);
        dd($users);

        if($users->is_subscribe ==0)
        {
            DB::table('users')->where('id','=',$id)->update(['is_subscribe' => "1"]);   
            return back()->with('success','Subscribe changed to Active !');
        }
        else
        {
            DB::table('users')->where('id','=',$id)->update(['is_subscribe' => "0"]);
            return back()->with('delete','Subscribe changed to Deactive !');
        }
    }
