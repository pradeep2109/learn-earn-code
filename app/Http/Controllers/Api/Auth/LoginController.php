    ...
    public function addDeviceToken(Request $request){
        if($request->user_id && $request->device_token){
            ...
            if($user){
                ...
                $user->save();
                return response()->json(['message' => "device token added"]);
            }else{
                return response()->json(['message' => "user not found"]);
            }
        }elseif(!$request->user_id){
            return response()->json(['message' => "user id required"]);
        }elseif(!$request->device_token){
            return response()->json(['message' => "device token required"]);
        }   
    }
